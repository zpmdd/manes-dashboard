...
function DataMapping({ item, result, onChange }) {
  ...
  const { mapped, error } = useMemo(() => {
    try {
      const mapped = getMappedData(result, binding, item);
      if (professionalTypes.includes(item.type)) buildProfessionalChart(item, mapped);
      return { mapped };
    } catch (issue) { return { error: issue.message }; }
  }, [result, binding, item.type, item.aggregate, item.unit, item.columns, item.rowCount, item.target]);
...
function ChartControls({ item, onChange }) {
  ...
  return <section className="ep-property-section"><h3>图表设置</h3><Select label="配色" value={value.palette} onChange={palette => change({ palette })}><option value="champagne">香槟暖色</option><option value="ocean">海洋蓝绿</option><option value="forest">森林青金</option></Select><div className="ep-field-grid"><Toggle label="显示图例" value={value.legend} onChange={legend => change({ legend })}/><Toggle label="数值标签" value={value.labels} onChange={labels => change({ labels })}/>{['multiLine','combo'].includes(item.type) && <Toggle label="平滑曲线" value={value.smooth} onChange={smooth => change({ smooth })}/>}{['multiLine','stacked','combo','scatter'].includes(item.type) && <Toggle label="区间缩放" value={value.zoom} onChange={zoom => change({ zoom })}/>}</div>{item.type === 'combo' && <><div className="ep-field-grid"><Field label="主系列名称" value={value.primaryName} maxLength={20} onChange={primaryName => change({ primaryName })}/><Field label="副系列名称" value={value.secondaryName} maxLength={20} onChange={secondaryName => change({ secondaryName })}/></div><Field label="副轴单位" value={value.secondaryUnit} maxLength={8} onChange={secondaryUnit => change({ secondaryUnit })}/></>}{item.type === 'scatter' && <div className="ep-field-grid"><Field label="X 轴名称" value={value.xName} maxLength={20} onChange={xName => change({ xName })}/><Field label="Y 轴名称" value={value.yName} maxLength={20} onChange={yName => change({ yName })}/></div>}</section>;
}
