...
import { normalizeConfig } from './dashboardConfig';
...
export function App() {
  ...
  return <div className={`editor-shell ${editing ? 'is-editing' : ''} ${editor.editing ? 'has-toolbar' : ''}`}>
    ...
    {editing && <aside className={`editor-library-pane ${sidePanel === 'library' ? 'is-open' : ''}`}><Suspense fallback={<p role="status">正在加载组件库…</p>}><ComponentLibrary onAdd={type => { editor.add(type); setSidePanel('inspector'); }}/></Suspense><section className="editor-layer-list" aria-label="图层列表"><h3>画布图层</h3>{[{ ...config.map, id: 'map', title: config.mapTitle }, ...config.modules].map(item => <button key={item.id} onClick={() => { editor.select(item.id); setSidePanel('inspector'); }} aria-pressed={editor.selected === item.id}><span>{item.title || '未命名组件'}</span><small>{item.locked ? '锁定' : !item.visible ? '隐藏' : ''}</small></button>)}</section></aside>}
    ...
    {config.modules.map(item => <CanvasItem key={item.id} id={item.id} title={item.title || '未命名组件'} item={item} editor={editor}><BoundWidget item={editing && !item.visible ? { ...item, visible: true } : item} result={results[item.binding.sourceId]} refresh={refresh} code={loaded?.code || NATIONAL} index={index} onNavigate={navigateWidget}/></CanvasItem>)}
    </div>
    ...
    {dialog === 'sources' && <Suspense fallback={<div className="toast" role="status">正在加载数据源管理…</div>}><DataSourcePanel sources={config.dataSources} code={code} onChange={applySources} onClose={() => setDialog(null)}/></Suspense>}
    ...
    {editing && <aside className={`editor-inspector-pane ${sidePanel === 'inspector' ? 'is-open' : ''}`}><Suspense fallback={<p role="status">正在加载组件属性…</p>}><ComponentInspector item={selection} isMap={editor.selected === 'map'} dataSources={config.dataSources} onChange={values => editor.patch(editor.selected, values)} onDuplicate={editor.duplicate} onDelete={editor.remove} onArrange={editor.arrange}/></Suspense></aside>}
  </div>;
}
